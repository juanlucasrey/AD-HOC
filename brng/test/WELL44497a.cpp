...
extern "C" {
#include "third_party/umontreal/WELL44497a.h"
}
...
auto main() -> int {
...

    {
        ...
        adhoc::WELL44497a<std::uint32_t> rng1(seq1);
        std::seed_seq seq2{1, 2, 3, 4, 5};
        adhoc::WELL44497a<std::uint64_t> rng2(seq2);
    ...
    {
        std::seed_seq seq{1, 2, 3, 4, 5};
        adhoc::WELL44497a<std::uint64_t> rng(seq);
        check_fwd_and_back(rng, 1000000);
        adhoc::WELL44497a<std::uint64_t> rng2(seq);
        EXPECT_EQUAL(rng, rng2);
    }
    ...
    static_assert(
        std::bidirectional_iterator<adhoc::WELL44497a<std::uint32_t>>);
