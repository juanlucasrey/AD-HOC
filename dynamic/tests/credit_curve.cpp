...
main() -> int
{
    ...
    auto [vals, span] = loadCSVToMatrix("expected_first_derivatives_vals.csv", 8, 8);
    expectedFirstDerivativesVals = std::move(vals);
    expectedFirstDerivatives = span;

    expectedSecondDerivativesVals = loadCSVToVector("expected_second_derivatives_vals.csv");
    adhoc::mdspan<const double, 3> expectedSecondDerivatives(expectedSecondDerivativesVals.data(), 8, 8, 8);
