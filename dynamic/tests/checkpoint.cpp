...
{
    std::vector<adhoc::Method> const methods = { adhoc::Method::Bwd,
...
test_simd8_backpropagation()
{
    ...
    // SIMD8 backpropagation - all 8 lanes at once
    {
        ...
        // Set method to SIMD8
        tape.configure(adhoc::Method::Bwd, 3, num_lanes);
    ...
    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
        ...
        tape.configure(adhoc::Method::Bwd, 3, 1, 1);
...
{
    std::vector<adhoc::Method> methods = { adhoc::Method::BwdBuffer,
                                           adhoc::Method::BwdBufferCompressed,
                                           adhoc::Method::BwdBufferPathReuse,
                                           adhoc::Method::BwdBufferCompressedPathReuse };
    auto const safe_method = methods[0];
...
test_lossy_compressed_univariate_non_reduced()
{
    ...
    for (auto m : methods) {
        ...
        auto const scEnd = log(inputD);
        auto ht = cos(scStart * scEnd);
        auto pvi = (scEnd * ht) * ht;
        auto outputD = pvi + inputD;
...
test_lossy_compressed_reset_registration()
{
    ...
    for (auto m : methods) {
        ...
        auto stop1 = tape.get_position();
        auto intermediary = inputD2 * (scEnd * 1.1);
        auto intermediary2 = cos(intermediary);
        tape.register_variable(inputD2);
        auto stop2 = tape.get_position();
        auto outputD = exp(cos(scEnd)) * log(intermediary) * intermediary2 * inputD2;

        tape.register_output_variable(outputD);

        tape.set_derivative(outputD, 1.0);
        tape.backpropagate_and_reset_to(stop2);
        tape.backpropagate_and_reset_to(stop1);
        tape.backpropagate();

        if (m == safe_method) {
            df = tape.get_derivative(inputD1);
            f = adhoc::passive_value(outputD);
        }
        else {
            auto df_2 = tape.get_derivative(inputD1);
            auto f2 = adhoc::passive_value(outputD);
            EXPECT_NEAR_ABS(df, df_2, 1e-13);
...
compute_result_branch_unused_ops(T x1, T x2, T x3, std::size_t num_paths) -> double
{
    ...
    for (std::size_t path = 0; path < num_paths; ++path) {
        ...
        T unused = exp(cos(y_path)) * x1;

        T res = x1 * (y_path * one_over_paths);
...
test_lossy_path_reuse_unusedops()
{
    ...
    for (auto m : methods) {
        ...
        if (m == safe_method) {
            res_adhoc = compute_result_branch_unused_ops(x1, x2, x3, num_paths);
        }
        else {
            double res_adhoc2 = compute_result_branch_unused_ops(x1, x2, x3, num_paths);
            EXPECT_NEAR_ABS(res_adhoc, res_adhoc2, 1e-13);
        }
...
compute_result_branch_invertedmult(T x1, T x2, T x3, std::size_t num_paths) -> double
{
    ...
    for (std::size_t path = 0; path < num_paths; ++path) {
        ...
        T res = exp(x1 * (y_path * one_over_paths)) * x2;
...
{
    std::vector<adhoc::Method> const methods = { adhoc::Method::BwdBufferCompressedPathReuse,
