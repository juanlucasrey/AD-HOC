...
struct Tape<Float>::Impl {
    std::variant<BackPropagator<Float>,
                 ...
                 BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP>,
                 BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP, true> >
...
Tape<Float>::set_method(Method m)
{
    if (m == Method::FirstOrderSimple) {
        ...
        this->impl->bp.template emplace<BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP> >();
    }
    else if (m == Method::SecondOrderVLossy) {
        this->impl->bp.template emplace<BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP, true> >();
    }
}
...
Tape<Float>::get_method() const -> Method
{
    ...
    if (std::holds_alternative<BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP> >(this->impl->bp)) {
        return Method::SecondOrderLossy;
    }

    if (std::holds_alternative<BackPropagator2Lossy<double, MapType::BOOST_UNORDERED_MAP, true> >(this->impl->bp)) {
