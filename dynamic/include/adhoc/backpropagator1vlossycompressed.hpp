#ifndef ADHOC_BACKPROPAGATOR1VLOSSYCOMPRESSED_HPP
#define ADHOC_BACKPROPAGATOR1VLOSSYCOMPRESSED_HPP
...
// BackPropagatorVLossyCompressed - creates and uses a LossyTape for backpropagation
template<class Float>
class BackPropagatorVLossyCompressed {
  private:
  ...
  public:
    explicit BackPropagatorVLossyCompressed() = default;
    ...
    void set_derivative(std::size_t var_id, double deriv, std::size_t /* ops_size */, std::size_t lane = 0)
    {
        ...
        this->output_derivs[std::distance(reg.begin(), it) * this->m_num_lanes + lane] = deriv;
    }
    ...
    auto get_derivative(std::size_t var_id, std::size_t lane) const -> double
    {
        ...
        return this->input_derivs[std::distance(reg.begin(), it) * this->m_num_lanes + lane];
    }
...
void
BackPropagatorVLossyCompressed<Float>::backpropagate_to(std::size_t to, TapeData& data)
{
    std::size_t from = data.next_id;
    std::size_t const num_lanes = this->m_num_lanes;
    ...
    std::vector<double> buffer(buffer_size_prev * num_lanes, 0.0);
    ...
    for (std::size_t i = 0; i < buffer_active_id.size(); ++i) {
        ...
        if (active_id != passive_id<std::size_t>) {
            if (active_id < from && active_id >= to) {
                std::size_t const dest_base = buffer_val_idx * num_lanes;
                std::size_t const src_base = i * num_lanes;

#pragma omp simd
                for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                    buffer[dest_base + lane] = buffer_active[src_base + lane];
                }
    ...
    for (std::size_t lossy_op_idx = 0; lossy_op_idx < this->lossy_op.size(); ++lossy_op_idx) {
        ...
        switch (op) {
            case LossyOpCode::ADD_FROM_SEED: {
                ...
                std::size_t const out_base = out_pos * num_lanes;
                std::size_t const in_base = in_pos * num_lanes;

                if (in_interface) {
                    // maybe this should throw. why copy form output to intput directly?
#pragma omp simd
                    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                        der_in[in_base + lane] += der_out[out_base + lane];
                    }
                }
                else {
#pragma omp simd
                    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                        buffer[in_base + lane] += der_out[out_base + lane];
                    }
                }
                break;
            }
...
#pragma omp simd
                for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                    buffer[in_base + lane] = buffer[out_base + lane];
                }
...
#pragma omp simd
                    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                        der_in[in_base + lane] += buffer[out_base + lane];
                    }
                }
                else {
#pragma omp simd
...
#pragma omp simd
            ...
            case LossyOpCode::MUL_INPLACE: {
                ...
                std::size_t const base = pos * num_lanes;
#pragma omp simd
                for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                    buffer[base + lane] *= multiplier;
                }
                break;
            }
...
#pragma omp simd
                    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                        der_in[in_base + lane] += multiplier * buffer[out_base + lane];
                    }
                }
                else {
#pragma omp simd
                    for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                        buffer[in_base + lane] += multiplier * buffer[out_base + lane];
                    }
                }
                break;
            }
...
#pragma omp simd
    ...
    for (std::size_t idx = 0; idx < buffer_id.size(); ++idx) {
        ...
        if (current_id != passive_id<std::size_t>) {
            ...
            std::size_t const idx_base = idx * num_lanes;
            if (loc != passive_id<std::size_t>) {
                std::size_t const loc_base = loc * num_lanes;
#pragma omp simd
                for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                    buffer_active[loc_base + lane] += buffer[idx_base + lane];
                }
            }
            else {
                std::size_t free_pos = 0;
                if (buffer_active_free_positions.empty()) {
                    ...
                    buffer_active.resize(buffer_active.size() + num_lanes, 0.0);
                ...
                std::size_t const free_pos_base = free_pos * num_lanes;
#pragma omp simd
                for (std::size_t lane = 0; lane < num_lanes; ++lane) {
                    buffer_active[free_pos_base + lane] = buffer[idx_base + lane];
                }
                loc = free_pos;
            }
        }
    ...
    if constexpr (Reset) {
        der_in.resize(der_in.size() - input_decrease_counter * num_lanes);
        this->input_ids.resize(this->input_ids.size() - input_decrease_counter);

        der_out.resize(der_out.size() - output_decrease_counter * num_lanes);
...
#endif // ADHOC_BACKPROPAGATOR1VLOSSYCOMPRESSED_HPP
