#ifndef FUNCTIONS_DISTRIBUTION_HPP
#define FUNCTIONS_DISTRIBUTION_HPP
...
// Note: we templatise normal cdf and pdf to allow for aad on qfdxva
// however the quantile function is not templatised yet.
// That will probably require a special implementation (if needed) since its
// derivative is a closed formula.
...
template <class Distribution, class D = double> struct pdf {};

template <class D> struct pdf<standard_normal, D> {
    static inline auto impl(D const &x) {
        using std::exp; // to allow Argument-dependent lookup
        constexpr double minus_half = -0.5;
        constexpr double pi_double = constants::pi<double>();
        constexpr double one_over_root_two_pi =
            1.0 / constants::sqrt(2.0 * pi_double);
        return one_over_root_two_pi * exp(x * x * minus_half);
    }
};

template <class Distribution, class D = double> struct cdf {};

template <class D> struct cdf<standard_normal, D> {
    static inline auto impl(D const &x) {
...
template <class Distribution, bool Precise, auto Method, bool CheckInput>
struct quantile {};
...
template <auto Method> struct quantile_sn_half_domain<true, Method> {
    static inline auto impl(double x) -> double {
        ...
        if (x >= 4.3458473798968777e-311) { // 2^(-1031)
            ...
            constexpr double pi_double = constants::pi<double>();
            constexpr double root_two_pi = constants::sqrt(2.0 * pi_double);
            x = (cdf<standard_normal>::impl(r) - x) * root_two_pi *
                std::exp(r * r * 0.5);
...
template <bool Precise, auto Method>
struct quantile<standard_normal, Precise, Method, false> {
    static inline auto impl(double x) -> double {
...
template <class Distribution, bool Precise, auto Method>
struct quantile<Distribution, Precise, Method, true> {
    static inline auto impl(double x) -> double {
        if (x > 1.0 || x < 0.0) {
            throw std::runtime_error("quantile input must be in [0,1]");
        }
        // QUANTMATH_THROW_IF(x > 1.0 || x < 0.0)
        // ("quantile input must be in [0,1]");
        ...
        return quantile<Distribution, Precise, Method, false>::impl(x);
    }
};

template <class Distribution, auto Method, class ForwardIt>
struct quantile_vec {
    static inline void impl(ForwardIt first, ForwardIt last) {
        // default implementation
        while (first != last) {
            detail::quantile<Distribution, false, Method, false>::impl(
                *first++);
        }
    }
};
...
struct quantile_vec<standard_normal, algorithm::bsm, ForwardIt> {
    static inline void impl(ForwardIt first, ForwardIt last) {
        constexpr double a0 = 2.50662823884;
        constexpr double a1 = -18.61500062529;
        constexpr double a2 = 41.39119773534;
...
template <class ForwardIt>
struct quantile_vec<standard_normal, algorithm::acklam, ForwardIt> {
    static inline void impl(ForwardIt first, ForwardIt last) {
        constexpr double a0 = 2.506628277459239;
        constexpr double a1 = -30.66479806614716;
        constexpr double a2 = 138.3577518672690;
...
template <class Distribution, class D = double> inline auto pdf(D const &x) {
    return detail::pdf<Distribution, D>::impl(x);
}

template <class Distribution, class D = double> inline auto cdf(D const &x) {
    return detail::cdf<Distribution, D>::impl(x);
}

template <class Distribution, bool Precise = false,
          auto Method = algorithm::bsm, bool CheckInput = false>
inline auto quantile(double x) -> double {
    return detail::quantile<Distribution, Precise, Method, CheckInput>::impl(x);
}

template <class Distribution, auto Method, class ForwardIt>
inline void quantile(ForwardIt first, ForwardIt last) {
    detail::quantile_vec<Distribution, Method, ForwardIt>::impl(first, last);
}

} // namespace adhoc2

#endif // FUNCTIONS_DISTRIBUTION_HPP
