...
#ifndef COMBINATORICS_MULTINOMIAL_COEFFICIENT_INDEX_SEQUENCE_HPP
#define COMBINATORICS_MULTINOMIAL_COEFFICIENT_INDEX_SEQUENCE_HPP
...
constexpr auto MultinomialCoeff2(std::index_sequence<I...> idx_seq)
    ...
    std::size_t res = factorial(order);
    std::size_t k = 0;
    while (k < bins) {
        res /= factorial(temp[k]);
        k++;
    }
    return res;
}
