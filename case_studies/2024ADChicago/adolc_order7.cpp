...
int main() {
    ...
    std::array<double, 330> results_average{};
    ...
    for (std::size_t j = 0; j < iters; ++j) {
        ...
        for (std::size_t dim6 = 0; dim6 < (n + 1); ++dim6) {
            multi[6] = dim6;
            for (std::size_t dim5 = dim6; dim5 < (n + 1); ++dim5) {
                multi[5] = dim5;
                for (std::size_t dim4 = dim5; dim4 < (n + 1); ++dim4) {
                    multi[4] = dim4;
                    for (std::size_t dim3 = dim4; dim3 < (n + 1); ++dim3) {
                        multi[3] = dim3;
                        for (std::size_t dim2 = dim3; dim2 < (n + 1); ++dim2) {
                            multi[2] = dim2;
                            for (std::size_t dim1 = dim2; dim1 < (n + 1);
                                 ++dim1) {
                                multi[1] = dim1;
                                for (std::size_t dim0 = dim1; dim0 < (n + 1);
                                     ++dim0) {
                                    multi[0] = dim0;
                                    results_average[counter++] +=
                                        tensor[0]
                                              [tensor_address(degree, multi)];
                                }
                            }
                        }
                    }
    ...
    std::cout << "ADOL-C order 7 time (ms): " << time << std::endl;
