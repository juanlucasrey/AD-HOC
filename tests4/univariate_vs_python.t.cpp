...
TEST(UnivariateFunctions, LGammaPrecision) {
    ...
    EXPECT_LT(std::get<2>(stats), 1e-15);
    ...
    for (std::size_t i = 1; i < 5; i++) {
        ...
        EXPECT_LT(std::get<0>(stats), errors[i - 1][0]);
        EXPECT_LT(std::get<1>(stats), errors[i - 1][1]);
        EXPECT_LT(std::get<2>(stats), errors[i - 1][2]);
    }
}
...
TEST(PythonCompare, ZetaPrecision) {
    ...
    std::vector<std::array<double, 3>> borwein_errors{
        ...
        {1e-15, 1e-14, 1e-13}, {1e-15, 1e-15, 1e-14}, {1e-16, 1e-15, 1e-14},
        {1e-16, 1e-15, 1e-14}, {1e-15, 1e-15, 1e-13}, {1e-16, 1e-15, 1e-14},
        {1e-15, 1e-15, 1e-14}};
